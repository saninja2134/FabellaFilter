...
# JPEG quality for training images (85 ≈ Roboflow compression, ~30-50 KB per 1024×1024)
JPEG_QUALITY = 85


def _to_uint8(img):
    """Convert any image to uint8 for JPEG encoding."""
...
class YoloPreparer:
    ...
    def _poly_to_bbox(coords):
        ...
        pts = augmentation.pair_points(coords)
        xs = [p[0] for p in pts]
        ys = [p[1] for p in pts]
    ...
    def convert_dicom_to_16bit_png(self, src_path, dst_path, device=None):
        # Delegates to DicomConverter for a single DICOM → 16-bit PNG.
        from converter import DicomConverter
        # convert_to_bone_png expects a directory; write a thin wrapper
        try:
    ...
    def setup_dataset(self, config=None, also_export_coco=True, progress_callback=None, step_callback=None):
        ...
        for f in labeled_files:
            ...
            if os.path.exists(png_path):
                ...
                labels_check = augmentation.read_labels(lbl_path)
                if labels_check:
        ...
        if config and config.get('augmentations', {}).get('mosaic', {}).get('enabled'):
            ...
            for img_path, lbl_path in pos_samples:
                ...
                if img is not None:
                    labels = augmentation.read_labels(lbl_path)
        ...
        def write_sample_files(img, labels, split, fname, original_source, is_augmented):
            ...
            if labels:
                ...
                if self.task == 'detect':
                    ...
                    augmentation.write_labels(lbl_dst, det_labels)
                else:
                    augmentation.write_labels(lbl_dst, labels)
            ...
            for img_path, lbl_path in samples:
                ...
                if lbl_path and config and multiplier > 1:
                    for k in range(1, multiplier):
                        ...
                        aug_labels = augmentation.read_labels(lbl_path)
                        # Preprocessing is applied inside write_sample_files; augment first
                        aug_img, aug_labels = augmentation.augment_sample(
...
"""
    ...
    def export_coco(self, log=print):
        ...
        # Wipe stale COCO data to prevent mixed-resolution images from
        # prior preparations lingering alongside newly-exported ones.
        for stale_dir in ["train", "valid", "test", "annotations"]:
            stale_path = os.path.join(coco_root, stale_dir)
        ...
        for split in ["train", "val"]:
            ...
            for fname in sorted(os.listdir(img_src)):
                ...
                labels  = augmentation.read_labels(lbl_path) if os.path.exists(lbl_path) else []
        ...
        for split in ['train', 'val']:
            ...
            for fname in os.listdir(seg_lbl_dir):
                if not fname.endswith('.txt'):
                    continue
                seg_labels = augmentation.read_labels(os.path.join(seg_lbl_dir, fname))
                det_labels = [(cid, self._poly_to_bbox(coords)) for cid, coords in seg_labels]
                augmentation.write_labels(os.path.join(det_lbl_dir, fname), det_labels)
